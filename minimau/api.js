...
const db = {
  ...
  fatura: { dia_fechamento: 15, dia_vencimento: 22, notificar: true },
...
export async function getSaldo(mes = db.hoje.mes) {
  ...
  // Ritmo do dia = orçamento do dia ANTES de gastar hoje; o restante desconta o dia.
  const hojeIso = `${mes}-${String(db.hoje.dia).padStart(2, "0")}`;
  const gasto_hoje = db.gastos.filter((g) => g.data_gasto.startsWith(hojeIso)).reduce((s, g) => s + g.valor, 0);
  const ritmo_diario = (disponivel + gasto_hoje) / dias_restantes;
  const ritmo_restante_hoje = ritmo_diario - gasto_hoje;

  const dias_para_fechar = db.fatura.dia_fechamento >= db.hoje.dia
    ? db.fatura.dia_fechamento - db.hoje.dia
    : db.hoje.dias_no_mes - db.hoje.dia + db.fatura.dia_fechamento;
  const total_ciclo = comprometido_total + gasto_livre;
  ...
  return {
    ...
    dias_restantes, ritmo_diario, gasto_hoje, ritmo_restante_hoje,
    fatura: { ...db.fatura, dias_para_fechar, total_ciclo },
    renda_definida: renda_total > 0,
  };
}

export async function definirFatura(campos) {
  if (!USAR_MOCK) return req("/api/fatura", { method: "PATCH", body: JSON.stringify(campos) });
  await espera(120);
  Object.assign(db.fatura, campos);
  return getSaldo();
}

// Painel do mês: categoria, dia a dia, semanas, maiores gastos e comparativo.
export async function getDashboard(mes = db.hoje.mes) {
  if (!USAR_MOCK) return req(`/api/dashboard?mes=${mes}`);
  await espera(200);
  const gastos = db.gastos.filter((g) => g.data_gasto.startsWith(mes));
  const total = gastos.reduce((s, g) => s + g.valor, 0);
  const dias_no_mes = db.hoje.dias_no_mes;

  const mapa = {};
  gastos.forEach((g) => { mapa[g.categoria] = (mapa[g.categoria] || 0) + g.valor; });
  const por_categoria = Object.entries(mapa)
    .map(([categoria, valor]) => ({ categoria, valor, percentual: total ? (valor / total) * 100 : 0 }))
    .sort((a, b) => b.valor - a.valor);

  const por_dia = Array.from({ length: dias_no_mes }, (_, i) => {
    const d = String(i + 1).padStart(2, "0");
    return { dia: i + 1, valor: gastos.filter((g) => g.data_gasto.startsWith(`${mes}-${d}`)).reduce((s, g) => s + g.valor, 0) };
  });
  const por_semana = [0, 1, 2, 3].map((i) => ({
    rotulo: `S${i + 1}`,
    valor: por_dia.slice(i * 7, i === 3 ? dias_no_mes : i * 7 + 7).reduce((s, d) => s + d.valor, 0),
  }));

  const anterior = somaMes(mes, -1);
  const total_anterior = db.gastos.filter((g) => g.data_gasto.startsWith(anterior)).reduce((s, g) => s + g.valor, 0);
  const maior_dia = por_dia.reduce((a, b) => (b.valor > a.valor ? b : a), por_dia[0]);

  return {
    total, media_diaria: total / db.hoje.dia,
    por_categoria, por_dia, por_semana, maior_dia,
    dias_no_mes, dias_com_gasto: por_dia.filter((d) => d.valor > 0).length,
    top: [...gastos].sort((a, b) => b.valor - a.valor).slice(0, 3),
    total_anterior, variacao: total_anterior ? ((total - total_anterior) / total_anterior) * 100 : null,
  };
}
